...
const videos=[
 ['One Piece — مغامرة جديدة','عرض تجريبي عبر YouTube','https://www.youtube.com/watch?v=QczGoCmX-pI'],
 ['Naruto — طريق النينجا','مشهد ترويجي','https://www.youtube.com/watch?v=22R0j8UKRzY'],
 ['Demon Slayer — مشهد سينمائي','عرض تجريبي','https://www.youtube.com/watch?v=VQGCKyvzIM4'],
 ['Jujutsu Kaisen — مواجهة','مشهد تجريبي قصير','https://www.youtube.com/watch?v=pkKu9hLT-t8'],
 ['My Hero Academia — الأبطال','عرض تجريبي','https://www.youtube.com/watch?v=Q7w7tK3hQhQ'],
 ['Attack on Titan — العرض','مقطع رسمي/ترويجي','https://www.youtube.com/watch?v=MGRm4IzK1SQ'],
 ['Bleach — Thousand-Year Blood War','مشهد ترويجي','https://www.youtube.com/watch?v=78WIYzX_m98'],
 ['Solo Leveling — بداية الصعود','عرض تجريبي','https://www.youtube.com/watch?v=JiaY9q8m3VQ']
];
...
const videoGrid=document.querySelector('#videoGrid');videoGrid.innerHTML=videos.map((v,i)=>{const id=v[2].split('v=')[1]?.split('&')[0]||'';return `<article class="video"><div class="thumb"><img loading="lazy" alt="${v[0]}" src="https://i.ytimg.com/vi/${id}/hqdefault.jpg" onerror="this.src='https://placehold.co/800x450/10243e/67c8ff?text=Anime+Station'"><a class="play" href="${v[2]}" target="_blank" rel="noopener" aria-label="تشغيل ${v[0]}">▶</a></div><h3>${v[0]}</h3><p>${v[1]}</p></article>`}).join('');
...
async function openDetail(item,type){
 ...
 content.innerHTML=`<div class="detail"><div class="poster"><img id="detailImg" alt="${item.title}"></div><div><span class="eyebrow">${type==='manga'?'MANGA':'ANIME'}</span><h2>${item.title}</h2><p><b>${item.meta}</b></p><p>${item.desc}</p><div class="detail-actions"><a class="primary" href="#videos" onclick="closeDetail()">شاهد الفيديوهات</a>${item.official?`<a class="secondary" href="${item.official}" target="_blank" rel="noopener">المصدر الرسمي</a>`:''}</div></div></div>`;
...
document.querySelector('#closeModal').onclick=closeDetail;modal.onclick=e=>{if(e.target===modal)closeDetail()};document.addEventListener('keydown',e=>{if(e.key==='Escape')closeDetail()});
...
// Give the hero a real cover from the same public anime data source when available.
